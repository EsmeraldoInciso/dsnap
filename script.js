...
const DPI = 300;
const SIZES = {
  '2x6':  { w: 2 * DPI, h: 6 * DPI,  label: '2x6"' },
  '4x6':  { w: 4 * DPI, h: 6 * DPI,  label: '4x6"' },
  '4x6L': { w: 6 * DPI, h: 4 * DPI,  label: '4x6" L' },
};

const LAYOUTS = [
  { id: '2x6_3', name: '2x6 Classic', desc: '3 Photos', size: '2x6', shots: 3, miniCols: 1, miniRows: 3,
  ...
  { id: '2x6_4', name: '2x6 Strip', desc: '4 Photos', size: '2x6', shots: 4, miniCols: 1, miniRows: 4,
  ...
  { id: '4x6_single', name: '4x6 Single', desc: '1 Photo', size: '4x6', shots: 1, miniCols: 1, miniRows: 1,
    getSlots: function(cw, ch) { var pad=cw*0.04; return [{x:pad,y:pad,w:cw-pad*2,h:ch*0.88-pad}]; } },
  { id: '4x6_3', name: '4x6 Triple', desc: '3 Photos', size: '4x6', shots: 3, miniCols: 1, miniRows: 3,
  ...
  { id: '4x6_4grid', name: '4x6 Grid', desc: '4 Photos', size: '4x6', shots: 4, miniCols: 2, miniRows: 2,
  ...
  { id: '4x6_collage', name: '4x6 Collage', desc: '3 Photos', size: '4x6', shots: 3, miniCols: 2, miniRows: 2,
  ...
  { id: '4x6L_2up', name: '4x6 Landscape', desc: '2 Photos', size: '4x6L', shots: 2, miniCols: 2, miniRows: 1,
  ...
  { id: '4x6_6grid', name: '4x6 Six-Up', desc: '6 Photos', size: '4x6', shots: 6, miniCols: 2, miniRows: 3,
        ...
        if (myCount < MAX_FIREBASE_GALLERY) {
          ...
          return db.collection('gallery').doc(preDocId).set(entry)
            .then(function() {
              ...
              showStatus('(cloud) Saved to cloud with QR!', 'success');
              startKioskTimer();
            });
        } else {
          ...
          lastSavedDocId = null;
          showStatus('(phone) Saved locally (cloud full)', 'info');
          startKioskTimer();
        }
      })
      .catch(function(err) {
        ...
        saveLocalGallery(entry);
        lastSavedDocId = null;
        showStatus('(phone) Saved locally', 'info');
        startKioskTimer();
      });
  } else {
    entry.createdAt = new Date().toISOString();
...
function saveLocalGallery(entry) {
  try {
    var key = 'dsnap_gallery';
    try { var u = firebase.auth().currentUser; if (u) key = 'dsnap_gallery_' + u.uid; } catch(e) {}
...
function renderFrames() {
  ...
  FRAMES.forEach(function(f) {
    var bg = f.bg || 'transparent', inner = f.id === 'none' ? 'X' : '';
...
function saveSettings() {
  var key = 'dsnap_settings';
  try { var u = firebase.auth().currentUser; if (u) key = 'dsnap_settings_' + u.uid; } catch(e) {}
  try { localStorage.setItem(key, JSON.stringify(state.settings)); } catch(e) {}
}
